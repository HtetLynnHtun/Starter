...
class MovieDetailViewController: UIViewController {
    ...
    private func fetchSimilarSeriess() {
        seriesModel.getSimilarSeries(id: contentId) { [weak self] result in
            ...
            case .success(let movieResults):
                self.similarSeries = movieResults
                self.collectionViewSimilarContents.reloadData()
            case .failure(let error):
                print(error)
            }
    ...
    // MARK: - Data bindings
    private func bindData(_ data: MovieResult) {
        ...
        if let releaseDate = data.releaseDate ?? data.firstAirDate,
           !releaseDate.isEmpty {
            labelReleaseYear.text = String(releaseDate.split(separator: "-")[0])
        } else {
            labelReleaseYear.text = "0000"
        }
        ...
        if let runtime = data.runtime,
           runtime > 0 {
            labelRuntime.text = readableRuntime(runtime)
        } else {
